 ...
 *
 * At runtime the stub reopens its own file, seeks past itself, reads the inner EXE's MZ
 * header, then streams the payload in 8 KB chunks and LZW-expands it into memory
 * (`int 21h/AH=3Fh` in a refill routine — hence "disk-streaming packer"). The compression
 * is the same codec as the .CC archives, so lzw.ts decodes it unchanged; the only real work
 * here is locating the stream and rebuilding a loadable EXE around it.
 *
 * Stub layout, offsets into the stub image (file offset minus the outer header size), read
 ...
 *
 * The inner MZ header is an ordinary DOS header; the stub takes CS:IP, SS:SP and the memory
 * sizing from it, and its e_lfarlc/e_crlc describe a relocation table of its own. In our
 * copy e_crlc is 0 (the EXEPACK stub self-relocates, which is why the output also declares
 * 0 relocations), so no fixups are applied. A copy with a nonzero count would need them
 * applied before the image is meaningful — that is refused rather than silently mishandled.
 */
...
/**
 * The image the copy this project was reversed against expands to. A mismatch is not fatal
 * here — the KBU2 hash gate is the real check — but it is worth saying loudly, because
 * every offset in patches.csv assumes this exact build of the game.
 */
const KNOWN_IMAGE_SHA256 =
  "06ca56b4d1ca737b050178cd394cc9e52e9879c860dcf51c45ff457cc5236c4a";
...
export const unpackNwc = (kbExe: Uint8Array): NwcResult => {
  ...
  // A 2-paragraph (32-byte) header, the layout unpackExepack expects. No relocations (crlc
  // was verified 0 above), entry and stack from the inner header.
