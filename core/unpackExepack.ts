/**
 * Strips EXEPACK, rebuilding the relocation table: the result is the edit base. Flat and
 * uncompressed, every string at a fixed offset, editable in place.
 *
 * The packer stub is identified by the signature "RB" at its CS:0x10, and by its error
 * string "Packed file is corrupt". Its 18-byte header at CS:0000 holds the real entry
 * CS:IP, SS:SP and the decompressed length.
 *
 ...
 * the last record.
 *
 * EXEPACK self-relocates at runtime from its own table, which is why the input runs at any
 * load address despite declaring 0 relocations in its DOS header. That table sits right
 * after the "Packed file is corrupt" string as 16 sections, each a uint16 count followed by
 * that many uint16 offsets; section i contributes segment base i*0x1000. It is decoded here
 * and written out as a real DOS relocation table, which is what makes the output an
 * ordinary loadable EXE.
 */

import { concat, equal, find, fromLatin1, setU16s, u16, u16s } from "./bytes.ts";

/**
 * Image offset of the packer stub's CS:0000, where the EXEPACK header sits: the stub's
 * entry is CS:IP = 1935:0012, IP 0x12 being just past that 18-byte header.
 */
const STUB = 0x19350;
...
export const unpackExepack = (packed: Uint8Array): Uint8Array => {
  // The load image exactly as the DOS header declares it, never the file size: the declared
  // length is what DOS would load, and what the EXEPACK offsets below assume. Any tail past
  ...
  let src = STUB;
  // 0xFF padding before the first record
  while (src > 0 && img[src - 1] === 0xff) src--;
  let dst = dest.length;
  for (;;) {
    const cmd = img[src - 1];
  ...
  const minalloc = Math.max(
    0x200,
    Math.floor((realSs * 16 + realSp - dest.length) / 16) + 0x200,
  );
