/**
 * The font commands: res/font.png <-> the 0x9bb2 member of a CC archive, and the build step
 * that writes both run-dir archives.
 *
 * res/font.png is the editable sheet: its top 8 rows are the game's original glyphs and its
 * bottom 8 the Cyrillic. font-export and font-import round-trip it, so an image editor's
 * own save is a valid input — which is why reading goes through pngjs: the sheet can come
 * back palette, greyscale, 16-bit or interlaced, and pngjs normalizes all of it to the
 * 8-bit RGBA core/font.ts wants.
 *
 * Files and messages only: core/font.ts knows what a glyph is, and takes raw RGBA so the
 * decoder stays platform-shaped: pngjs here, createImageBitmap in a browser, one
 * sheetToMember behind both.
 */
...
/**
 * Whatever the member holds: 256 glyphs for ours, 128 for a pristine archive. Exporting 128
 * from a 256-glyph font drops the Cyrillic half and writes a sheet too short to import
 * back, so `glyphs` overrides — which is how a stock archive yields a 256-cell sheet with a
 * blank upper half to draw into.
 */
