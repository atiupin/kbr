...
KB_F.EXE      *** the translation base *** — fully unpacked, flat, editable. Keep pristine.
KBR.EXE       *** our working copy *** — KB_F + the one-byte protection patch. The
              runnable/testable build (regenerate via patch_protection.py, see below).
QWE.DAT       an in-game save sitting in the game dir (that is where KB reads/writes them).
...
tools/        everything hand-written: DOS utils (CUP386.COM, LOADFIX.COM), the Python
              tools, ghidra.sh, and ghidra_scripts/ (our Ghidra analysis scripts).
dumps/        extracted data + annotated listings (see "Dumps").
build/        generated, safe to delete: KB_U.EXE (CUP386 intermediate), decomp/ (Ghidra
              output), ida/ (leftover IDA database from the abandoned IDA experiment).
...
1. **Outer NWC layer** — a ~1.2 KB custom stub that streams the packed payload from disk
   ...
   (generic code-tracing unpacker) inside DOSBox: `CUP386 KB.EXE KB_U.EXE /1 /x`.
   → produces `build/KB_U.EXE` (still EXEPACK-packed inside).
2. **Inner EXEPACK layer** — Microsoft EXEPACK, signature `"RB"` at the stub's `CS:0x10`,
   ...
   table lives in the packed tail), which is why `KB_U.EXE` runs at any load address despite
   showing **0 relocations** in its DOS header. Removed statically by `tools/unpack_exepack.py`.
   → produces `KB_F.EXE`: flat image + a proper **1960-entry DOS relocation table**.

Rebuild KB_F from scratch: `python3 tools/unpack_exepack.py build/KB_U.EXE KB_F.EXE`
(re-derive KB_U by running the CUP386 command above in DOSBox-X.)

### Validation

`KB_F.EXE`'s decompressed image matches the **live running game** (`dumps/memdump_base1.bin`)
**98.9%** after applying relocations at load segment `0x892`; the ~1% delta is runtime-mutated
variables — expected. Confirmed at runtime by the user: `KB_F` runs, **and `LOADFIX KB_F` runs**
(DOS now relocates it correctly at a shifted address). Both reach deep game logic (the
copy-protection prompt), proving the unpack is sound.
...
- Location in `KB_F`: one contiguous block; strings sit at fixed offsets, editable in place.
...
- **CUP386 v3.4** (archive.org item `CUP386`). Generic unpacker. **Gotcha: fails "unable to load
  source file" if given paths — use bare filenames in the same dir.** Tracers `/1` fast, `/3`
  V86, `/7` full emulator (slow, most robust); `/x` = force EXE output.
...
- **Comments truncate in the Listing by default.** The EOL comment field is narrow and clips
  ...
  `AnnotateKbCom.java 0` forces every comment into a pre-comment. `ShowComments.java` prints a
  program's comments in the terminal, to check annotation work without the GUI.
...
- **Importing `KB!.COM` into Ghidra** (a `.COM` has no header, so nothing auto-detects):
  ...
  Headless equivalent: `tools/ghidra.sh import 'KB!.COM' -loader BinaryLoader
...
across all game logic. Translation stays **binary patching** of `KB_F.EXE`. Where genuinely new
...
**verification branch**, ~1.1 KB later inside an unanalyzed block. They were easy to confuse:
the prompt routine really does contain no comparison, which is why an early pass wrongly
concluded the check was cosmetic. It is not — see `KB!.COM` below.
...
`KB_F.EXE`) directly and the branch stays `JC`, the check runs, and the game exits.

### Where it lands in `KB_F.EXE`
...
```
python3 tools/patch_protection.py KB_F.EXE -o KBR.EXE     # -> KBR.EXE, 1 byte differs
```

`KB_F.EXE` stays pristine; `KBR.EXE` is our working copy / runnable-testable build. **Re-apply
after any regeneration via `unpack_exepack.py`.**

### The prompt routine

**`FUN_19fe_000b`** @ `19fe:000b`, **file offset `0xBFEB`**, length `0x2E3`. Called from `main`
(`FUN_1507_0009` @ file `0x7079`) at `1507:004c`, between init and the game loop.

1. Seeds from the DOS clock — `INT 21h/AH=2Ch` (twice), plus `AH=2Ah` (date).
2. Random byte `AND 0xFE` (forces even) indexes a **128-entry `uint16` table in the code
   segment** at `19fe:02EE` = **file `0xC2CE`**, 256 bytes, sitting immediately after the
   function's `RETF`.
3. Deobfuscates the entry: `v = entry - 0x2C3; v ^= 0xD393; ROR v,1 ×3; RCR v,1`, then unpacks
   bitfields — `page = v & 0x3F`, `line = (v>>6) & 0x1F`, `word = (v>>11) & 0x0F`.
4. Prints the prompt (strings at DS `0xAD6`, `0xAEB`, `0xAF3`, `0xAF9`, `0xB0C`, `0xB29`,
   `0xB48`, `0xB67`, `0xB85`, `0xBA0`), then reads ≤16 chars via `FUN_1207_26b8` (plain line
   editor: backspace, ESC cancels, Enter accepts).
5. Returns **without comparing anything** — no `CMP`, and the typed buffer is just a stack local
   at `[BP-0x24]`. It hands back `DX:AX` = far pointer to the typed word and `BX` = the raw
   table value. `main` ignores both. The verdict is reached later, at the `0xC40A` branch.
   _Do not mistake this for "no protection" — that was the earlier wrong call._

Decode verified: all 128 entries yield page 4–61, line 1–30, word 1–11 with **no zero** in any
field — a wrong algorithm would produce zeros. Full list: `dumps/protection_table.txt`.

The routine *is* deliberately obfuscated (junk register arithmetic at `19fe:0017-004e`; an
`STC; JC` always-taken pair at `19fe:001f` hopping over 13 garbage bytes at `19fe:002f-003b` to
derail linear disassemblers; `PUSH AX / PUSH BP / MOV BP,SP / MOV [BP+2],imm / POP BP` instead of
plain pushes) — but the verification step simply isn't there. All ten prompt strings resolve to
this one function and nowhere else.

The prompt's ten strings are ordinary UI text and still need translating. The gate itself is the
`0xC40A` branch, handled by the patch above.

## Dumps

- `dumps/memdump_base1.bin` — original game running (normal launch), program image at linear
  `0x8920`, load segment `0x892`.
- `dumps/memdump_loadfix.bin` — original under LOADFIX, image at `0x12C77`, load segment `0x10C0`
  (= base1 + `0x82E` paragraphs). The two-address pair is what lets us diff out relocations.
...
- [x] Defeat both packing layers → `KB_F.EXE`, a flat editable base that runs (incl. LOADFIX).
- [x] **Copy protection** — fully traced. `KB!.COM` is a loader-patcher that flips one byte at
      ...
      producing `KBR.EXE`. See section above. _Still to confirm in DOSBox-X that `KBR` runs
      standalone where `KB_F` exits._
- [ ] **Cyrillic font** — game font is Latin-only. Find the font bitmap (in `KB_F` or a `.CC`
      member), add glyphs, remap to CP866. Nothing renders in Russian until this exists.
- [ ] **Translation** — edit strings in `KB_F` respecting the two length limits; build repointing
      only for the strings that can't be shortened.
- [ ] **Patcher** — ownership-gated installer producing the translated build.

## Conventions

- `KB_F.EXE` is the single source of truth for edits. Regenerate via the script; don't hand-hack
  headers. The game needs its `.CC` files in the same directory to run.
- Keep original `KB.EXE` / `KB!.COM` / `.CC` pristine (never modify — the patcher references them).
