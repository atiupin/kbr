...
```
game/         *** pristine originals, supplied by the user *** — never modified; everything
              is built FROM them. Untracked in git (only game/README.md is), because they
              are not ours to distribute; the directory ships empty.
...
build/        *** generated, safe to delete *** — also the RUN DIR (DOSBox mounts it as C:).
              Every game-derived byte lives here, so none of it is tracked in git:
  KB_NWC.EXE    intermediate: KB.EXE minus the outer NWC packer (tools/unpack_nwc.py).
  KBU.EXE       *** the translation base *** — fully unpacked, flat, editable. Keep
                pristine; regenerated by tools/unpack_exepack.py.
  ...
  256.CC 416.CC working copies — the game reads its .CC from the current directory, so
                KBR.EXE needs them beside it. These carry the Cyrillic-extended font
                (member 0x9bb2), so they differ from the pristine game/ copies. Rebuild:
                `python3 tools/cc.py font-import tools/font.png game/256.CC build/256.CC`
                (and the same for 416.CC).
  *.DAT         in-game saves (QWE/WE/ASD.DAT — where KB reads/writes them, fixed 20,421 B).
  decomp/       Ghidra output (see "Source language & decompilation").

ghidra/       *** the Ghidra tooling *** — hand-written, tracked. No game data here:
  ghidra.py        the front-end (gui / run / import) — see "Tooling".
  scripts/         the analysis scripts it runs (*.java).
tools/        the build, all hand-written Python: unpack_nwc.py, unpack_exepack.py,
              apply_patches.py + the patch manifest patches.csv, cc.py, find_ref.py,
              addr.py, and the editable font sheet font.png. Stdlib only, no DOS
              utilities — full inventory under "Tooling".
tmp/          *** scratch, gitignored *** — nothing here is source:
  KBR.gpr KBR.rep  the Ghidra project database. Game-derived and fully regenerable, so it
                   lives in scratch; `ghidra/ghidra.py` points here. Rebuild it per
                   "Rebuilding the Ghidra project".
  kbcom_annotated.asm  the hand-annotated KB!.COM listing — kept out of git by choice (the
                   protection is solved and the findings live in "Copy protection" below).
                   Recover the last tracked copy with `git show a44da0b:dumps/kbcom_annotated.asm`.
  (captures)       DOSBox-X screenshots and MEMDUMP.BIN land here.
dosbox-x.conf DOSBox-X config: mounts build/ as C: — the only drive, game files only.
```

Convention: **`tools/` builds, `ghidra/` analyses, `build/` is generated (and is the run
dir), `game/` is the user's own untouched originals, `tmp/` is scratch.** Nothing precious
lives in `build/` or `tmp/` — but both hold work that takes real time to recreate (the build,
the Ghidra project), so delete them deliberately, not by reflex.
...
**Both layers are now stripped by Python; nothing in the build needs DOS.**

1. **Outer NWC layer** — a 1177-byte custom stub that reopens its own file, streams the packed
   payload in 8 KB chunks (`int 21h/AH=3Fh` — hence "disk-streaming packer") and expands it. No
   standard packer signature, which is why it first fell to CUP386, a generic code-tracing
   unpacker run inside DOSBox. It has since been reversed properly: the codec is **the same LZW
   as the `.CC` archives** (variable-width, LSB-first, 9→12 bits, clear `0x100`, end `0x101`,
   first code `0x102`, GIF-style growth), so `tools/cc.py`'s decoder decodes it unchanged. The
   file layout is `[1177-byte stub][inner MZ header][LZW stream]`, with the inner header's offset
   stored at stub `+0x4b` (`0x699`) and the stream starting right after it. Stripped by
   **`tools/unpack_nwc.py`**.
   → produces `build/KB_NWC.EXE` (still EXEPACK-packed inside). Its image was **byte-identical to
   CUP386's** across all 107,501 bytes — that equality is what retired CUP386, which has been
   deleted from `tools/` (it is in git history if a future packed binary ever needs it).
2. **Inner EXEPACK layer** — Microsoft EXEPACK, signature `"RB"` at the stub's `CS:0x10`,
   ...
   → produces `build/KBU.EXE`: flat image + a proper **1960-entry DOS relocation table**.

Full rebuild from the user's own `game/` copy — five commands, plain Python, no DOSBox:
...
Verified end to end: starting from `game/KB.EXE` these reproduce `build/KBU.EXE` at its gated
SHA-256 (`a0ad8832…`) and the same `KBR.EXE` as before. DOSBox-X is now only needed to **run**
the game, never to build it. **Dependencies: none** — every step is Python 3 stdlib, including
the font sheet's PNG read/write (`cc.py` does it on `zlib`). Verified by running the whole
five-command build from an empty `build/` with `PIL` import blocked: identical hashes.
...
the unpack is sound. (`LOADFIX` is a DOSBox-X built-in — it runs a program above the first 64 KB,
which is both the classic EXEPACK-at-low-address workaround and how the second, shifted memory
dump was taken. No `LOADFIX.COM` is needed in the repo.)
...
- Volume: **~2,650 words of prose**, ~740 unique phrase lines, plus menus/items/spells.
  (Text extracts are not kept in the repo — they are game data. Re-extract from
  `build/KBU.EXE` with `strings`/`tools/find_ref.py` when needed.)
...
- **DOSBox-X** (`brew install --cask dosbox-x`; Gatekeeper quarantine stripped) — needed only to
  **run and debug** the game; the build itself is pure Python. Debug build
  with `MEMDUMPBIN [seg]:[off] [len]` → writes `MEMDUMP.BIN` into the captures dir, `tmp/`
  (screenshots land there too). Launch:
  `/Applications/dosbox-x.app/Contents/MacOS/dosbox-x -conf dosbox-x.conf`.
  C: is `build/` and is the only drive — game files only. `LOADFIX` is a DOSBox-X built-in
  (`LOADFIX KBR` runs the build above the first 64 KB), so nothing else needs mounting.
...
- **`tools/unpack_nwc.py`** — strips the outer NWC packer: `game/KB.EXE` → `build/KB_NWC.EXE`
  (no args). Reuses `cc.py`'s LZW decoder; the stub layout it relies on is documented in its
  docstring. Refuses an inner header declaring relocations (ours declares none) and warns if the
  unpacked image doesn't hash like the copy this project was reversed against.
- **ndisasm** (`brew install nasm`) for 16-bit disassembly; objdump/lldb also present. This is
  what read the NWC stub (`ndisasm -b16 -o0 stub.bin`) when the outer packer was reversed.
- **`tools/cc.py`** — NWC "CC" archive tool, stdlib only. `list`,
  ...
  LZW codec above; validated round-trip on every member of both archives. Rebuilding the two
  run-dir archives from the originals is exactly two calls (verified byte-identical to the
  current `build/` copies):
  ```
  python3 tools/cc.py font-import tools/font.png game/256.CC build/256.CC
  python3 tools/cc.py font-import tools/font.png game/416.CC build/416.CC
  ```
  **PNG I/O is home-grown** (`png_read`/`png_write`, on `zlib`) so no Pillow is needed. Writing
  is fixed at 8-bit RGBA; reading accepts what an editor may hand back — colour types
  grey/RGB/palette/grey+alpha/RGBA, bit depths 1/2/4/8/16, `tRNS` transparency, all five row
  filters. Tested against Pillow-saved variants of each; **interlaced files are refused**, not
  silently misread, so re-save without interlacing if an editor insists on it.
- **`tools/apply_patches.py`** — manifest-driven, hash-gated builder of `build/KBR.EXE` from
  pristine `build/KBU.EXE` (no args). Patch types `bytes` / `string` / `reloc` (see "String repointing"). Run
...
- **Ghidra** (`brew install ghidra`; 12.1.2 at time of writing, but no version is pinned
  anywhere) — the analysis tool for this project. Drive it via
  **`ghidra/ghidra.py`**, which presets the project, program and script paths:

  ```
  ghidra/ghidra.py gui                          # open the GUI on tmp/KBR.gpr
  ghidra/ghidra.py run DumpAsm.java 1507:0009   # headless, output already de-noised
  ghidra/ghidra.py import <file> [opts]         # add another binary to the project
  ```

  Three commands, each earning its place: `run` wires up six analyzeHeadless flags and strips
  Ghidra's INFO noise from script output, `import` is the rebuild path, `gui` knows where the
  project moved to. (There was a `list` too — dropped; `ls ghidra/scripts` does the same, and
  each script's first comment line says what it does.)

  The **project database lives in `tmp/`** (scratch), not next to the script — `gui` and `run`
  check for it and print the rebuild command if it is missing, and `import` creates `tmp/` if
  needed. **Ghidra allows a single writer.** `run`/`import` refuse to start while the GUI holds
  the lock and say so; close it with File → Close Project (no need to quit). The install is
  located at runtime (`GHIDRA_HOME`, else `ghidraRun` on PATH, else Homebrew's stable
  `opt/ghidra` symlink) — **never pin a Cellar path with the version in it**, it breaks on the
  next `brew upgrade ghidra`. Homebrew's `ghidraRun` sets JAVA_HOME itself; headless gets a
  default of `/opt/homebrew/opt/openjdk@21` unless the environment already has one.
...
- **Ghidra GUI orientation**: Listing = disassembly, Decompiler = C-ish pseudocode, Window →
  ...
  _Manage Script Directories_ → add `ghidra/scripts`.
- **`ghidra/scripts/` inventory** (each is `-postScript`-style; run via `ghidra/ghidra.py
run <script> [args]` headless, or from the GUI Script Manager):
  ...
  - **`AnnotateKbCom.java [cutoff]`** — disassembles and annotates `KB!.COM`, replacing the
    ...
    listing: `tmp/kbcom_annotated.asm` (scratch — see "Extracted data & listings").
...
- **Importing `KB!.COM` into Ghidra** (a `.COM` has no header, so nothing auto-detects):
  ...
  Headless equivalent: `ghidra/ghidra.py import 'game/KB!.COM' -loader BinaryLoader
-loader-baseAddr 1000:0100 -processor 'x86:LE:16:Real Mode' -cspec default`
  ...
  **Gotcha: Ghidra rejects project paths containing a dot-directory** (`~/.claude/...` fails
  with "Path element starting with '.' is not permitted") — which is why the project sits in
  the repo's `tmp/` rather than in a hidden cache dir.
  Re-dump everything: `ghidra/ghidra.py run DumpDecomp.java build/decomp`.

### Rebuilding the Ghidra project (it lives in `tmp/`, gitignored)

`tmp/KBR.gpr` + `tmp/KBR.rep/` are **scratch, not source**: 4.3 MB of game-derived disassembly
holding **no hand-made annotation**. Audited with a throwaway script — the game binary carries 0
comments, 0 user-defined symbols and 0 user-named functions of 443; `KB!.COM`'s 31 comments and
18 symbols are all written by `AnnotateKbCom.java`, which is tracked and deterministic. **Do not
re-assert that the project holds irreplaceable annotation** — it did not. If you _do_ start annotating by hand in the GUI,
that changes and the decision needs revisiting.

To recreate it (needs `build/KBU.EXE`, so run the build first):

```
ghidra/ghidra.py import build/KBU.EXE                       # then let auto-analysis run
ghidra/ghidra.py import 'game/KB!.COM' -loader BinaryLoader \
    -loader-baseAddr 1000:0100 -processor 'x86:LE:16:Real Mode' -cspec default
ghidra/ghidra.py run AnnotateKbCom.java                     # -process it (see above)
```

The program must end up named **`KBU.EXE`** — that is what `ghidra.py`'s `PROGRAM` selects with
`-process`. (An earlier import was left named `KB_F.EXE` from a pre-rename file; headless then
matched nothing and quietly processed whatever it found. Renamed in place.)

Consequence of living in scratch: **`tmp/` is no longer safe to wipe blindly** — deleting it
costs the Ghidra project (minutes of auto-analysis to rebuild), though nothing irreplaceable.
...
`ghidra/scripts/DisasmRange.java` rather than expecting auto-analysis to find it.

Apply statically as one entry in the patch manifest (`type: bytes`, `offset: 0xC40A`,
`expect: 72`, `write: EB`). The generic engine `tools/apply_patches.py` builds `build/KBR.EXE`
from pristine `build/KBU.EXE`, gated on `KBU`'s SHA-256 (baked into the script) so every offset is
...
```
python3 tools/apply_patches.py        # build/KBU.EXE -> build/KBR.EXE (every patches.csv row)
```
...
page/line/word triple (decode them with `DisasmRange.java` if ever needed again), prints the prompt, and
...
## Extracted data & listings

Extracted game data is **not kept in the repo** — it is the game's own content, and all of it is
re-derivable from `build/KBU.EXE`. What used to live in `dumps/` (text lists, the 128 decoded
protection triples, the NWC stub) is in git history if ever needed; regenerate rather than
resurrect. Nothing derived is tracked any more:

- `tmp/kbcom_annotated.asm` — all 284 bytes of `KB!.COM` disassembled with a comment on every
  line, plus prose headers explaining each stage. **Hand-written and NOT regenerable**
  (`AnnotateKbCom.java` writes equivalent but terser comments into the Ghidra project, not this
  document) — moved to scratch deliberately: the protection is fully solved, the conclusions are
  written up under "Copy protection" below, and the build no longer depends on any of it. Its
  last tracked copy: `git show a44da0b:dumps/kbcom_annotated.asm`. If the launcher ever needs
  re-examining, read it from there rather than re-deriving it.
...
RAM snapshots (deleted after they served the unpack validation; regenerate via `MEMDUMPBIN`, they
land in `tmp/`): _base1_ = normal launch, image at linear `0x8920`, load segment `0x892`; _loadfix_
...
- [x] **Python-only build** — the outer NWC packer is reversed (it is the `.CC` LZW codec) and
      stripped by `tools/unpack_nwc.py`, byte-identical to CUP386's output. The whole chain
      `game/KB.EXE → KB_NWC → KBU → KBR` plus the `.CC` font import runs in any Python 3 env,
      **stdlib only** (`cc.py` does its own PNG I/O on `zlib`, so not even Pillow). DOSBox-X is
      only needed to play/debug, and the DOS utilities are gone from the repo: CUP386 is
      superseded and `LOADFIX` is a DOSBox-X built-in.
...
- [x] **Cyrillic font** — done and functional. CC member `0x9bb2` extended to **256 glyphs /
      ...
      repacked into both `256.CC`/`416.CC` (the `build/` run-dir copies differ from `game/`). The glyph
...
- `build/KBU.EXE` is the single source of truth for edits — regenerate `build/KBR.EXE` via the
  script, don't hand-hack headers. The game needs its `.CC` files in the run directory
  (`build/`, mounted as C:) to launch.
- Keep the pristine `game/` originals untouched. Rebuild the run-dir `.CC` copies from them with
  ...
  `game/` in place.
- **Nothing game-derived is tracked in git** — not the binaries, not a disassembly of them.
  `game/` (the user's own files), `build/` (everything we generate from them) and the Ghidra
  project database are all gitignored; the repo carries only hand-written things: the Python
  tools, the patch manifest, the font sheet, the Ghidra scripts + front-end, and docs. Two
  directories are kept present by a single tracked file each — `game/README.md` for the user's
  copy, and `ghidra/scripts/` for the analysis work.
- **The build stays pure Python 3 stdlib**, so it runs anywhere without a setup step: no
  third-party packages, no DOS utilities, no DOSBox. Reach for a dependency only if something
  genuinely cannot be written here — reversing the format has twice been the cheaper answer
  (the NWC packer instead of CUP386, PNG I/O instead of Pillow).
